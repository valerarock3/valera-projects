...
1. Установите **Node.js** (nodejs.org) и **MySQL 8.4** (dev.mysql.com).
2. Скачайте проект (ZIP или `git clone`).
3. **Запустите `start.bat`** двойным кликом.

Скрипт сам:
- установит зависимости (`npm install`), если их нет;
- найдёт MySQL и **создаст базу** `courses_db` и пользователя `app` (один раз спросит пароль root);
- поднимет оба сервера в отдельных окнах и откроет браузер;
- запустит phpMyAdmin, если установлен PHP.

Первый запуск сделает всё сам: таблицы, массажные курсы с уроками, админ и пользователь создаются автоматически.
